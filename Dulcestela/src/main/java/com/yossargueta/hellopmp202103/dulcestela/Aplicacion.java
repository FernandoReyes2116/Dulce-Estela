...
public class Aplicacion {
    ...
    public Aplicacion(Scanner EntradaTeclado) {
        ...
        this._MiCurso = new ArrayList<EstelaItem>();  
    }
        ...
        switch(opcionMenu.toUpperCase()){
          case "M":
            ...
            case "E": 
                ...
                break;
            case "S":
                System.out.println("Usted ha salido del sistema Gracias !!!! >.<");
                break;
            default:
    ...
    private void ingresarNuevoRegistro(){
        ...
        EstelaItem nuevoCursoItem =  new EstelaItem();
        nuevoCursoItem.setNombre(Layout.obtenerValorParaCampo("Nombre del Curso de cocina", "Nivel 1,2,3", this._EntradaTeclado));
        nuevoCursoItem.setNombre(Layout.obtenerValorParaCampo("Cedula", "0000-0000-00000", this._EntradaTeclado));
        nuevoCursoItem.setPrecio(Layout.obtenerValorParaCampo("Precio del Curso", "cantidad en L.", this._EntradaTeclado));
        nuevoCursoItem.setDuracion(Layout.obtenerValorParaCampo("Duracion del Curso", "en meses ", this._EntradaTeclado));
        int anio = Integer.parseInt(Layout.obtenerValorParaCampo("Año de creacion ", "ejemplo:2020", this._EntradaTeclado));
    ...
    private void mostrarRegistros(){
        ...
        try {
            ...
            for(int i = 0; i< this._MiCurso.size(); i++){
                columnas = ((EstelaItem) this._MiCurso.get(i)).obtenerCampos();
                Layout.imprimirEnColumna(columnas, anchos, "||");
            }
    ...
    private void actualizarRegistro(){
        ...
        EstelaItem selectCurso = null;
        for( int i=0; i < this._MiCurso.size(); i++){
            if( selectedId == ((EstelaItem)this._MiCurso.get(i)).getId()) {
                selectCurso = (EstelaItem)this._MiCurso.get(i);
                break;
            }
        }
        if (selectCurso == null ) {
